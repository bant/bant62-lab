...
#define IR_INIT_XMIT()      /* Tx: Initialize Transmitter (Timer0 for IR subcarrier: Fast PWM, clk/8) */ \
    ...
    TCCR0B = _BV(WGM02)|0b010
#else
#error "F_CPU are not declared."
#endif

#if   F_CPU== 8000000UL
#define IR_TX_38K()     OCR0A = 26; TCNT0 = 0   /* Tx: Set IR burst frequency to 38kHz */
#define IR_TX_40K()     OCR0A = 25; TCNT0 = 0   /* Tx: Set IR burst frequency to 40kHz */
#elif F_CPU==10000000UL
...
#elif F_CPU==12000000UL
#define IR_TX_38K()     OCR0A = 39; TCNT0 = 0   /* Tx: Set IR burst frequency to 38kHz */
#define IR_TX_40K()     OCR0A = 37; TCNT0 = 0   /* Tx: Set IR burst frequency to 40kHz */
#elif F_CPU==16000000UL
#define IR_TX_38K()     OCR0A = 52; TCNT0 = 0   /* Tx: Set IR burst frequency to 38kHz */
#define IR_TX_40K()     OCR0A = 50; TCNT0 = 0   /* Tx: Set IR burst frequency to 40kHz */
#else
#error "F_CPU are not declared."
#endif
...
#define IR_TX_ON()      TCCR0A |= _BV(COM0B1)   /* Tx: Start IR burst ”äŠrˆê’v‚ÅOC0BËßÝ LowÚÍÞÙo—Í */
...
#define IR_CAPT_ENA()   TIFR = _BV(ICF1); TIMSK = _BV(ICIE1)  /* Rx: Enable captureing interrupt */
...
#define IR_COMP_ENA(n)  OCR1A = TCNT1 + (n); TIFR = _BV(OCF1A); TIMSK |= _BV(OCIE1A)  /* Enable compare interrupt n count after now */
#define IR_COMP_DIS()   TIMSK &= ~_BV(OCIE1A)  /* Disable compare interrupt */
#define IR_COMP_NEXT(n) OCR1A += (n)            /* Tx: Increase compare register by n count */

#elif defined(__AVR_ATmega48__) ||defined(__AVR_ATmega88__) || defined(__AVR_ATmega168__) || defined(__AVR_ATmega48P__) || defined(__AVR_ATmega88P__) || defined(__AVR_ATmega168P__) || defined(__AVR_ATmega328P__)

#define IR_TX_ON()      TCCR0A |= _BV(COM0B1)   /* Tx: Start IR burst */
#define IR_TX_OFF()     TCCR0A &= ~_BV(COM0B1)  /* Tx: Stop IR burst */
#define IR_TX_TEST()    TCCR0A & _BV(COM0B1)    /* Tx: Check if IR is being transmitted or not */
...
#else
#error "CPU Type are not declared."
#endif

/* Counter clock rate and register width */
#if   F_CPU== 8000000UL
#define T_CLK           1000                   /* Timer tick period [ns] */
#elif F_CPU==10000000UL
#define T_CLK           800                    /* Timer tick period [ns] */
#elif F_CPU==12000000UL
#define T_CLK           666                    /* Timer tick period [ns] */
#elif F_CPU==16000000UL
...
ISR_CAPTURE()
{
    ...
    pw = pw1;
    pw1 = ct;
    ct -= pw2;          /* pw: mark length, ct: space length */
