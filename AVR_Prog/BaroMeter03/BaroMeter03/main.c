...
#define FONT_SIZE	8
#define IVT_SYNC	900			/* 900秒(15分)*/

//#define ADC_VREF		1.1					// [V] 内蔵VREF(1.1V)
//#define ADC_DIV_RATE		(100000.0/1100000.0)
//#define ADC_DIV_RATE		( 97900.0/1077000.0)
#define ADC_VREF		3.286					// [V] 内蔵VREF(1.1V)
//#define ADC_DIV_RATE		(100000.0/200000.0)
...
#define BATTERY_LEVEL4		3700
#define BATTERY_LEVEL3		3650
#define BATTERY_LEVEL2		3600
#define BATTERY_LEVEL1		3550
...
short atom_data[BUFSIZE];
short temperature_data[BUFSIZE];
short volt_data[BUFSIZE];
short ilum_data[BUFSIZE];
...
const custom_char_t eCustom[] EEMEM =
{
{{ 0x08 , 0x0C , 0x0E , 0x0F , 0x0E , 0x0C , 0x08 , 0x00}},		//カスタム文字[シリアル出力]
{{ 0x10 , 0x18 , 0x14 , 0x14 , 0x02 , 0x05 , 0x06 , 0x04}},		//カスタム文字[hp]
{{ 0x08 , 0x14 , 0x08 , 0x06 , 0x09 , 0x08 , 0x09 , 0x06}},		//カスタム文字[℃]
{{ 0x02 , 0x03 , 0x02 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00}},		//カスタム文字[AM]
{{ 0x00 , 0x00 , 0x00 , 0x00 , 0x02 , 0x03 , 0x02 , 0x00}},		//カスタム文字[PM]
{{ 0x04 , 0x15 , 0x0E , 0x1F , 0x0E , 0x15 , 0x04 , 0x00}},		//カスタム文字[晴]
{{ 0x00 , 0x0C , 0x1E , 0x1F , 0x1F , 0x0E , 0x07 , 0x00}},		//カスタム文字[雲]
{{ 0x04 , 0x0E , 0x1F , 0x04 , 0x04 , 0x05 , 0x02 , 0x00}}      //カスタム文字[雨]
};
...
ISR( TIMER2_OVF_vect )
{
    ...
    /* Sync request timer */
    {
	    static uint16_t ivt_sync;

	    if (++ivt_sync >= IVT_SYNC)
	    {
		    ivt_sync = 0;
		    isSync = true;
	    }
...
/*	エラー表示(無限ループ)				*/
/*======================================*/
void dispMessage2(const char* title, const *msg)
{
	ST7032i_Clear();
	ST7032i_setCursor(0 , 0);
    ST7032i_puts_p(title);
...
int main(void)
{
	...
	while(1)
	{
		...
		if ( BUTTON2 )		/* ボタン1が押された*/
		{
			...
			ST7032i_setCursor(0 , 0);
			//                    ﾃﾞﾝﾁ => x.xxxV
			xsprintf_p(buf, PSTR("\xC3\xDE\xDD\xC1 => %d.%dV"),volt/1000,volt%1000);
			ST7032i_puts(buf);
		...
		switch(state)
		{
			...
			case S_OPEN:
	            ...
	            pre_min = rtc_time.min;
	            pre_day = rtc_time.day;

				xsprintf_p(Fname, PSTR("%02u%02u%02u.LOG"), rtc_time.year % 100, rtc_time.month, rtc_time.day);
				// ファイルをオープン
			...
			case S_MEASUREMENT:
				...
				/* 日が変わったらファイルを閉じて新規ファイルを開く*/
				if((pre_day != rtc_time.day) && isLog)
				{
					pre_day = rtc_time.day;
				...
				if ( averageMpl115a2Data(&atmo_tmp1, &atmo_tmp2, &temp_tmp1, &temp_tmp2) )
				{
					...
					if (isLog)
					{
						if (pre_min != rtc_time.min)
						{
						    f_printf(&File1, "%04d-%02d-%02d,%02d:%02d:%02d,%d.%d,%d.%d,%d,%d.%d\n",
										rtc_time.year,rtc_time.month,rtc_time.day,
										rtc_time.hour,rtc_time.min,rtc_time.sec,
										atmo_tmp1, atmo_tmp2,
										temp_tmp1, temp_tmp2,
										ilum, volt/1000, volt%1000);
							pre_min = rtc_time.min;
